...
struct SnekAI_PMan_MSourek : public SnekAI
{
	...
	void FindTreats(const Board& board, const Snek& snek, Dir& moveRequest, bool& boost)
	{
		...
		PMMS::HraciPole hraciPole;
		hraciPole.FillBoard(board);

		PMMS::FloodFillFinder fff(hraciPole, snek.Body[0]);
		int attempts = 0;
		...
		while (fff.FloodFill() && attempts < 10)
		{
			...
			PMMS::HraciPole testPole(hraciPole);
			testPole.FillSnek(path);
			PMMS::FloodFillFinder fff2(testPole, fff.m_endPos);
			if (fff2.FloodFill())
			{
				// parada, uspech
			...
			{
				hraciPole(fff.m_endPos) = PMMS::EOccupiedBy::None;
			}
	...
	void Panik(const Board& board, const Snek& snek, Dir& moveRequest, bool& boost)
	{
		...
		{
			Coord nextCoord = PMMS::GetCoordInDir(head, dir);
